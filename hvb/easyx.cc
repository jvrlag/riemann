...
void EXDestroy(EXWindow *EW)
{
     if (EW->use_buffer)
	  XFreePixmap(EXInfo.display,EW->buffer);
     XDestroyWindow(EXInfo.display,EW->window);
}
...
void EXColor(const char *colorname)
{
     palette color=EXAllocNamedColor(colorname);
     EXSetColor(color);
}

void EXColor(double R, double G, double B)
{
     palette color=EXAllocRGBColor(R,G,B);
     EXSetColor(color);
}
...
// 3 -> COPY, usual one
// 7 -> OR, 
void EXMode(int mode)
{
     XGCValues xgcvalues;
     xgcvalues.function=mode;
     XChangeGC(EXInfo.display,EXInfo.gc,GCFunction,&xgcvalues);
}
