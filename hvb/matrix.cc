...
void Vector::Remove(long k)
{
     if (N==1) { Destroy(); return; }
     memmove(D+k,D+k+1,(N-k)*sizeof(double));
...
{
     char form[40];
     sprintf(form,"%%%ld.%ldg ",prec+4,prec);
     for (long i=1;i<=N;i++)
	  printf(form,D[i]);
     printf("\n\n");
}
...
long Vector::Max_Index(long k) const
{
     if (k>N) merror("Asking for Max_Index(k) with k>N\n");
     List Removed;
     for (long l=1;l<=k;l++)
     {
	  double E0=-DBL_MAX;
	  long imax=0;
	  for (long i=1;i<=N;i++)
	       if (!Removed.Find(i) && D[i]>E0) { E0=D[i]; imax=i; }
	  Removed.Append(imax);
     }
     return Removed(k);
}

long Vector::Min_Index(long k) const
{
     if (k>N) merror("Asking for Min_Index(k) with k>N\n");
     List Removed;
     for (long l=1;l<=k;l++)
     {
	  double E0=DBL_MAX;
	  long imin=0;
	  for (long i=1;i<=N;i++)
	       if (!Removed.Find(i) && D[i]<E0) { E0=D[i]; imin=i; }
	  Removed.Append(imin);
     }
     return Removed(k);
}
