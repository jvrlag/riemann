...
void CVector::Remove(long k)
{
     if (N==1) { Destroy(); return; }
     memmove(D+k,D+k+1,(N-k)*sizeof(cmplx));
     D=(cmplx*)realloc(D,N*sizeof(cmplx));
     N--;
#ifdef DEBUG
     total_memory--;
#endif
...
void CMatrix::Create(long n1, long n2) // n2=0
{
     Destroy();
     N1=n1; N2=n2;
     if (!N2) N2=N1;
...
CMatrix Part(const CMatrix &A, const List &L1, const List &L2)
{
     CMatrix B(L1.N,L2.N);
...
#endif
     long *I=(long*)malloc(N1*sizeof(long));
     CMatrix B(*this);
     B.LU_Decomp(I);
     int signo=1;
     for (long i=1;i<=N1;i++)
...
long CMatrix::Kernel(CMatrix &K) const
{
     ...
     long *indx=(long*)malloc((N1+1)*sizeof(long)); 
     CMatrix A(*this);
     A.LU_Decomp(indx);
     free(indx);
...
int CMatrix::Save(FILE *fich) const
{
     ...
     for (long i=1;i<=N1;i++)
     {
	  ...
	  {
	       cmplx x=Elem(i,j);
	       fprintf(fich,"(%16.12g,%16.12g) ",real(x),imag(x));
	  }
	  fprintf(fich,"\n");
     }
